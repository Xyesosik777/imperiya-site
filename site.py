...
.features{display:flex;flex-direction:column;align-items:center;gap:60px;margin-bottom:120px}
...
footer{width:100%;text-align:center;margin:60px 0 18px 0;color:#888;font-size:1.05rem;letter-spacing:.5px;opacity:.7}
...
<h2 class="section-title">Почему стоит купить именно наш клиент?</h2>
...
def profile():
    ...
    conn = get_db(); conn.row_factory = sqlite3.Row
    row  = conn.execute("""SELECT uid,username,email,hwid,subscription_end,role,banned
                           FROM users WHERE username=?""", (session['user'],)).fetchone()
    conn.close()
    if row and row['banned']:
        session.clear()
        flash("Ваш аккаунт заблокирован!")
        return redirect('/login')
    return render(PROFILE_HTML, user=row)
